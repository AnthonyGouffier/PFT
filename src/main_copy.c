...
/**/
SDL_Rect ArenaZone = {(1920-(1920*0.6))/2,0,1080,AREA_HIGH};

// Déclaration de la fonction afficherCarte pour l'utiliser plus tard
...
void afficherCarteBoutique(SDL_Renderer* renderer, pokemon_t* pokemon) {
    for (int i = 0; i < NUM_BUTTONS; i++) {
        if (CartePkm[i].visible) {
            afficherCarte(renderer, &CartePkm[i].rect, pokemon);
        }
    }
}
...
int main(int argc, char* argv[]) {
    ...
    afficherCarteBoutique(renderer, pokemon);
    // Boucle principale
    SDL_Event event;
    bool quit = false;
